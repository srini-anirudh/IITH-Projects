...
def pad_text(batch, pad_id=0):
  ...
  inst_batch = torch.Tensor(inst_batch).long()
  mask_batch = torch.Tensor(mask_batch).float()
...
def pad_vector(batch, vec_len):
  ...
  inst_batch = torch.Tensor(inst_batch).float()
  mask_batch = torch.Tensor(mask_batch).float()
...
def run_condense(x_data, tokenizer, encoder, model):
  ...
  for x_batch in x_data:
    ...
    sum_tokens = torch.zeros(vocab_size, hidden_dim)
    cnt_tokens = torch.zeros(vocab_size)
...
def bert_label_smoothing(y_batch, tokenizer, language_model, rate=0.1):
  ...
  pad = torch.zeros(batch_size, seq_len, 1)
