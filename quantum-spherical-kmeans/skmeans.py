...
class SKMeans:
    ...
    def run_kmeans(self, input_matrix, delta=0.001):
        """
        ...
        if input_dimensions != centre_dimensions:
            raise ValueError(
                "Number of dimensions in input samples and centres should be same"
            )
        ...
        for i in range(self.iters):
            if self.metric == "euclidean":
                self.distances = euclideanDistance(
                    input_matrix, self.centres, normalised=True
                )
            elif self.metric == "cosine":
                self.distances = 1 - (cosineSimilarity(input_matrix, self.centres))
            elif self.metric == "linear_combination":
                self.distances = self.alpha * euclideanDistance(
                    input_matrix, self.centres, normalised=True
                ) + self.beta * (1 - cosineSimilarity(input_matrix, self.centres))
            else:
            ...
            self.distances = (
                np.ones(input_samples) - self.distances[input_seq, self.labels]
            )
            avg_distance = self.distances.mean()
            try:
                if (1 - delta) * prev_distance <= avg_distance <= prev_distance:
                    break
    ...
    def fit(self, input_matrix, sample=True, param_centres=None, two_pass=False):
        ...
        """
        # input_matrix = input_matrix / norm(input_matrix, axis=1, keepdims=True)
